...
#else
    // JD: There seems to be a bug in the above code. In case of arrays and objects
    // toks should be incremented once more (+2 instead of +1).
    JSON_API struct json_token* json_obj_next(struct json_token* toks)
    {
        if (toks[1].type == JSON_ARRAY || toks[1].type == JSON_OBJECT) {
            toks = (toks + 2) + toks[1].sub;
        } else {
            toks = toks + 2;
        }
        return toks;
